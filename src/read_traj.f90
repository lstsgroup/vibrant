...
!********************************************************************************************
    ...
    SUBROUTINE read_normal_modes(gs, sys, stats)! natom, element, normal_freq_file, normal_displ_file, freq, disp, nmodes, read_function, type_static, force_file, force)
...
!********************************************************************************************
    ...
    SUBROUTINE read_static(gs, sys, dips, rams)!sys%natom, sys%element, rams%static_pol_file, rams%pol, dips%static_dip_file, gs%spectral_type%%type_dipole, dips%static_dip, type_static)
        ! Variables of your derived types:
        TYPE(global_settings), INTENT(INOUT)   :: gs
        TYPE(systems), INTENT(INOUT)        :: sys
        ...
        !CHARACTER(LEN=40), INTENT(IN)                               :: rams%static_pol_file, dips%static_dip_file
        !CHARACTER(LEN=40), INTENT(IN)                               :: type_static, gs%spectral_type%%type_dipole
        !INTEGER, INTENT(INOUT)                                      :: sys%natom
        !CHARACTER(LEN=2), DIMENSION(:), ALLOCATABLE, INTENT(INOUT)    :: sys%element
        !REAL(kind=dp), DIMENSION(:, :, :, :, :), ALLOCATABLE, INTENT(OUT)  :: rams%pol
        !REAL(kind=dp), DIMENSION(:, :, :, :), ALLOCATABLE, INTENT(OUT)    :: dips%static_dip

        CHARACTER(LEN=40)                                          :: chara
        INTEGER                                                    :: i, j, k, m, n
        INTEGER                                                    :: stat

        ALLOCATE (rams%pol(sys%natom, 3, 2, 3, 3), dips%static_dip(sys%natom, 3, 2, 3))
                ...
                DO k = 1, 2
                    DO i = 1, sys%natom
                        DO j = 1, 3
                            ...
                            READ (52, *)
                            READ (52, *)
                            READ (52, *) chara, chara, chara, rams%pol(i, j, k, 1, 1), rams%pol(i, j, k, 2, 2), rams%pol(i, j, k, 3, 3)
                            READ (52, *) chara, chara, chara, rams%pol(i, j, k, 1, 2), rams%pol(i, j, k, 1, 3), rams%pol(i, j, k, 2, 3)
                            READ (52, *) chara, chara, chara, rams%pol(i, j, k, 2, 1), rams%pol(i, j, k, 3, 1), rams%pol(i, j, k, 3, 2)
                        END DO
                    END DO
                END DO
            END DO
                ...
                DO k = 1, 2
                    DO i = 1, sys%natom
                        DO j = 1, 3
                            READ (53, *, END=994)
                            READ (53, *)
                            READ (53, *) chara, dips%static_dip(i, j, k, 1), dips%static_dip(i, j, k, 2), dips%static_dip(i, j, k, 3)
                        END DO
                    END DO
                END DO
            END DO
...
!SUBROUTINE read_static(natom, element, static_pol_file, pol, static_dip_file, type_dipole, static_dip, type_static)
!
