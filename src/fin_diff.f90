   ...
   SUBROUTINE finite_diff_static(gs, sys, stats, dips, rams)
      ...
      IF (gs%spectral_type%read_function == 'R') THEN
         ...
         IF (dips%type_dipole == 'berry') THEN
            rams%pol(:, :, :, 1, :) = REAL((rams%static_dip_x(:, :, :, :) - &
                                            dips%static_dip(:, :, :, :))/(5.338d-5*1.313d-26), kind=dp)
            rams%pol(:, :, :, 2, :) = REAL((rams%static_dip_y(:, :, :, :) - &
                                            dips%static_dip(:, :, :, :))/(5.338d-5*1.313d-26), kind=dp)
            rams%pol(:, :, :, 3, :) = REAL((rams%static_dip_z(:, :, :, :) - &
                                            dips%static_dip(:, :, :, :))/(5.338d-5*1.313d-26), kind=dp)
      ...
      DO j = 1, stats%nmodes
         DO k = 1, sys%natom
            DO i = 1, 3
               IF (gs%spectral_type%read_function == 'R') THEN
                  rams%pol_dq(j, :, :) = rams%pol_dq(j, :, :) + &
                                         pol_dxyz(k, i, :, :)*stats%disp(j, k, i)*sys%atom_mass_inv_sqrt(k)
               ELSEIF (gs%spectral_type%read_function == 'IR') THEN
                  dips%dip_dq(j, :) = dips%dip_dq(j, :) + &
                                      dip_dxyz(k, i, :)*stats%disp(j, k, i)*sys%atom_mass_inv_sqrt(k)
               END IF
            END DO
         END DO
      END DO
