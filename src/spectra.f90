...
!    IF (sys%system=='1' .OR. (sys%system=='2' .AND. dips%type_dipole=='wannier')) THEN  !!fragment approach or the whole cell
      ...
      !!conversion of IR units to K*cm*km*mol^-1
        ir_const = avo_num*md%dt*fs2s*2.0_dp*10.0_dp/(12.0_dp*const_permit*speed_light*const_boltz)

        OPEN (UNIT=61, FILE='IR_spectrum.txt', STATUS='unknown', IOSTAT=stat) !!write output
        DO i = 0, 2*md%t_cor - 1
            freq(i) = i*freq_res
            ir_int(i) = REAL(md%zhat(i), kind=dp)*ir_const*(sinc_const*(i)/SIN(sinc_const*(i)))**2._dp
            IF (freq(i).GE.5000_dp) CYCLE
            ir_int(0) = 0.00_dp
...
!!!****************************************************************************************!
    SUBROUTINE spec_raman(gs, sys, md, dips, rams)
        ...
        REAL(kind=dp), DIMENSION(:), ALLOCATABLE                    :: raman_const, freq !sinc_func
...
!!!ACF AND FFT CALC!!!
      ...
      !!!Find the frequency resolution in cm^-1
        ...
        f = freq_res*md%dt*1.883652d-4 !MAGIC NUMBER
...
!!!PARALLEL!!!
        ...
        DO i = 0, 2*md%t_cor - 2
            ...
            raman_para(i) = (REAL(zhat_iso(i), kind=dp) + (REAL(zhat_aniso(i), kind=dp)*4.0_dp/45.0_dp))*raman_const(i)
            raman_para(0) = 0.0_dp
            IF (freq(i).GE.5000.0_dp) CYCLE
...
!                    CALL forward_diff(sys%mol_num, rams%e_fieldgit(xyz)%alpha_xyz, dip_free,rams%e_field(xyz)%dip_xyz, gs, sys)
!                ELSEIF (dips%type_dipole=='berry') THEN
!                    CALL forward_diff(sys%mol_num, rams%e_field(xyz)%alpha_xyz, dip_free, md%coord_v, gs, sys)
!                END IF
...
!....................................................................................................................!
    ...
    SUBROUTINE spec_static_ir(gs, stats, dips)
...
!....................................................................................................................!
    SUBROUTINE spec_static_raman(gs, sys, stats, rams)
...
!....................................................................................................................!
    ...
    SUBROUTINE spec_abs(gs, sys, dips, rams)
        ...
        TYPE(dipoles), INTENT(INOUT)        :: dips
        TYPE(raman), INTENT(INOUT)        :: rams
        INTEGER                                                       :: stat, i, j, k, m, o, dims, dir
        INTEGER(kind=dp)                                               :: plan
        ...
        IF (gs%spectral_type%read_function=='RR') THEN
            dims = 3
            dir = 2
        ELSE !gs%spectral_type%read_function == 'ABS'
            dims = 1
            dir = 1
            sys%natom = 1
        END IF
            ...
            DO j = 1, sys%natom
                DO i = 1, dims
                    DO k = 1, dir
                        DO m = 1, 3
                            DO o = 1, 3
                                CALL interpolate(rams%RR%zhat_pol_rtp(j, i, k, m, o, 1:rams%RR%framecount_rtp), &
...
!!....................................................................................................................!
!
    SUBROUTINE spec_resraman() ! natom, framecount, element, rtp_dipole_x, rtp_dipole_y, rtp_dipole_z, type_input, mol_num, system, read_function, dt, z_iso_resraman, z_aniso_resraman, freq_range, freq_range_rtp, laser_in_resraman, y_out)
!
