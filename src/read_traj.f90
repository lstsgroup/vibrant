            ...
            DO k = 1, 2
               DO i = 1, sys%natom
                  DO j = 1, 3
                     ...
                     READ (52, *)
                     READ (52, *)
                     READ (52, *) chara, chara, chara, rams%pol(1,1)%atom(i)%displacement(k)%XYZ(j), &
                        rams%pol(2, 2)%atom(i)%displacement(k)%XYZ(j), rams%pol(3, 3)%atom(i)%displacement(k)%XYZ(j)
                     READ (52, *) chara, chara, chara, rams%pol(1,2)%atom(i)%displacement(k)%XYZ(j), &
                        rams%pol(1, 3)%atom(i)%displacement(k)%XYZ(j), rams%pol(2, 3)%atom(i)%displacement(k)%XYZ(j)
                     READ (52, *) chara, chara, chara, rams%pol(2,1)%atom(i)%displacement(k)%XYZ(j), &
                        rams%pol(3, 1)%atom(i)%displacement(k)%XYZ(j), rams%pol(3, 2)%atom(i)%displacement(k)%XYZ(j)
                  END DO
               END DO
            END DO
         END DO
