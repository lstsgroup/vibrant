...
print*,nfrag,'nfrag check'
        ...
        *(((laser_in-((i)*freq_range))**4)/((i)*freq_range))*(1.0_dp/(1.0_dp-EXP((-1.438777_dp*&
           ((i)*freq_range))/temp))))*2.0_dp*2.0_dp*pi!*((-1.438777_dp*i*freq_range)/temp)*(1.0_dp/(1.0_dp-EXP((-1.438777_dp*&
                      !((i)*freq_range))/temp)))
         ...
         *(((laser_in-((i)*freq_range))**4)/((i)*freq_range))*(1.0_dp/(1.0_dp-EXP((-1.438777_dp*&
          ((i)*freq_range))/temp))))*2.0_dp*2.0_dp*pi
...
!!!X-FIELD!!!
       ...
       ALLOCATE(zhat_para(0:t_cor*2),zhat_unpol_x(0:t_cor*2),zhat_ortho(0:t_cor*2),zhat_depol_x(0:t_cor*2))
        ...
        DO i=0,2*t_cor-2
         ...
         zhat_ortho(i)=REAL(zhat_ortho(i),kind=dp)*((const_planck)/(8.0_dp*const_boltz*const_permit*const_permit)*1d-29*0.421_dp*dt&
                  *(((laser_in-((i)*freq_range))**4)/((i)*freq_range))*(1.0_dp/(1.0_dp-EXP((-1.438777_dp*&
                ((i)*freq_range))/temp))))*2.0_dp*pi*2.0_dp
        ...
        DO i=0,2*t_cor-2
         ...
         zhat_para(i)=REAL(zhat_para(i),kind=dp)*((const_planck)/(8.0_dp*const_boltz*const_permit*const_permit)*1d-29*0.421_dp*dt&
                 *(((laser_in-((i)*freq_range))**4)/((i)*freq_range))*(1.0_dp/(1.0_dp-EXP((-1.438777_dp*&
                 ((i)*freq_range))/temp))))*2.0_dp*pi*2.0_dp    
         zhat_para(0)=0.0_dp
        IF ((i*freq_range).GE.5000_dp) CYCLE
...
DO k=1,nmodes
 ...
 +((pol_dq(k,3,3)-pol_dq(k,1,1))**2.0_dp)))+(3.0_dp*((pol_dq(k,1,2)**2.0_dp)+(pol_dq(k,2,3)**2.0_dp)&
 +(pol_dq(k,3,1)**2.0_dp)))
ENDDO

!!!Calculation of the intensities!!
DO k=1,nmodes
 raman_int(k)=REAL(((7.0_dp*aniso_sq(k))+(45.0_dp*iso_sq(k)))/45.0_dp,kind=dp)*REAL(((laser_in-freq(k))**4.0_dp)/freq(k),kind=dp)&
  ...
  DO m=1,3
   DO o=1,3
    DO n=1,framecount_rtp 
     IF (check_pade=='n') THEN
      zhat_pol_dxyz_rtp(j,i,m,o,n)=REAL((REAL(zhat_pol_rtp(j,i,2,m,o,n),kind=dp)-REAL(zhat_pol_rtp(j,i,1,m,o,n),&
              kind=dp))*factor,kind=dp)
     ELSEIF (check_pade=='y') THEN
      zhat_pol_dxyz_rtp(j,i,m,o,n)=REAL((REAL(y_out(j,i,2,m,o,n),kind=dp)-REAL(y_out(j,i,1,m,o,n),kind=dp))*factor,kind=dp)
     ENDIF
    ENDDO
   ENDDO
  ENDDO
...
DO k=1,nmodes
 DO o=1,framecount_rtp
  iso_sq(k,o)=REAL((zhat_pol_dq_rtp(k,1,1,o)+zhat_pol_dq_rtp(k,2,2,o)+zhat_pol_dq_rtp(k,3,3,o))/3.0_dp,kind=dp)**2.0_dp
        
  aniso_sq(k,o)=(0.50_dp*(((zhat_pol_dq_rtp(k,1,1,o)-zhat_pol_dq_rtp(k,2,2,o))**2.0_dp)+&
  ((zhat_pol_dq_rtp(k,2,2,o)-zhat_pol_dq_rtp(k,3,3,o))**2.0_dp)+((zhat_pol_dq_rtp(k,3,3,o)-&
  zhat_pol_dq_rtp(k,1,1,o))**2.0_dp)))+(3.0_dp*((zhat_pol_dq_rtp(k,1,2,o)**2.0_dp)+&
  (zhat_pol_dq_rtp(k,2,3,o)**2.0_dp)+(zhat_pol_dq_rtp(k,3,1,o)**2.0_dp)))
 ENDDO
ENDDO
...
DO k=1,nmodes
 ...
 REAL(((laser_in_resraman-freq(k))**4.0_dp)/freq(k),kind=dp)*REAL(1.0_dp/(1.0_dp-EXP(-1.438777_dp*freq(k)/temp)),kind=dp)
ENDDO
...
DO x=start_freq,end_freq
 broad=0.0_dp
 DO i=1,nmodes
  broad=broad+(raman_int(i,rtp_point)*(1.0_dp/(omega*SQRT(2.0_dp*pi)))*EXP(-0.50_dp*((x-freq(i))/omega)**2.0_dp))
 ENDDO
...
DO i=0,2*t_cor-2
  ! j=22
   ...
   zhat_unpol_resraman(i,j)=(zhat_iso_resraman(i,j))+(zhat_aniso_resraman(i,j)*7.0_dp/45.0_dp)*&
           ((const_planck)/(8.0_dp*const_boltz*const_permit*const_permit)*1d-29*0.421_dp*dt*((((laser_in*j)-((i)*freq_range))**4)/&
           ((i)*freq_range))*(1.0_dp/(1.0_dp-EXP((-1.438777_dp*((i)*freq_range))/temp))))*2.0_dp*2.0_dp*pi
