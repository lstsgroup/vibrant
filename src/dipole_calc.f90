MODULE dipole_calc
    ...
    PUBLIC ::  center_mass, wannier_frag, solv_frag_index !,wannier, wannier_frag_old,center_mass_old
    PRIVATE

CONTAINS

    SUBROUTINE center_mass(filename, fragment, sys, md, dips)
...
!***************************************************************************************************
    ...
    SUBROUTINE solv_frag_index(filename, natom_frag, fragment, sys, md)
...
!!***************************************************************************************************
!!***************************************************************************************************
    SUBROUTINE wannier_frag(natom_frag, dipole, fragment, sys, md, dips)
        ...
        DO m = 1, sys%framecount
            DO i = 1, sys%mol_num
                DO j = 1, natom_frag(i)
                    ...
                    ELSEIF (dips%type_dipole=='1' .AND. sys%system=='2') THEN
                        CALL pbc_orthorombic_old(md%coord_v(m, fragment(m, i, j), :), sys%fragments%refpoint(m, 1, :), sys%cell%vec, sys%cell%vec_pbc, pox_x, pox_y, pox_z) !! IF COM is for whole supercel!!!
